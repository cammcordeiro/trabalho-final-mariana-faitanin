---
title: "Atividade_3_5_Final_Mariana_Faitanin - para essa atividade, usei a espécie T. spirictusantensis"
...
# baixar ocorrencias
trigo_gbif <- occ_data(scientificName = "Paratecoma peroba", 
                      hasCoordinate = TRUE,
...
trigo_gbif1 <- trigo_gbif$data %>%
  dplyr::select(scientificName, acceptedScientificName, decimalLatitude, decimalLongitude,
         ...
         datasetName, recordedBy, locality, organismID) 
```

#Temos 36 variáveis, mas com a função distinct do pacote dplyr conseguimos avaliar quais dessas são únicas.
...
#Note que das 36 variáveis iniciais restaram 24 ocorrências, isso acontece devido ao a quantidade de colunas iniciais que foram descartadas para essa tarefa. 
...
ggplot() +
  ...
  labs(x = "longitude", y = "latitude", title = expression(italic("Paratecoma peroba")))
```
...
trigo %>% 
  ...
  addMarkers(~decimalLongitude,
             ~decimalLatitude)
```
#A Paratecoma peroba é uma espécies endêmica do Brasil, com ocorrência Sudeste (Espírito Santo, Minas Gerais, Rio de Janeiro, São Paulo). Através do gráfico plotado com os dados de latitude e ocorrência, podemos perceber que essa espécie apresenta uma distribição dispersa entre os graus -25 a -10.Enretando, quando plotamos os mesmo pontos de ocorrência para a variável longitude, podemos observar que a ditribuição dessa espécie apresenta um pico entre os graus -45 a -40, região correspondente a Mata Atlântica e a parte da caatinga e cerrado.
O numero de coleta e a dificuldade na identificação podem estar influenciando nos dados para a real distribuição da espécies no Brasil.
```{r}
trigo %>% 
...
```
#ATENÇÃO: Paratecoma peroba e Tecoma peroba são Sinônimos e Variante ortográfica, por isso foi considerado os dois.

#Exportar os dados
```{r}
library(writexl)
write_xlsx(trigo_gbif1,"C:\\ciencia-colab\\Atividade-final\\trab-final\\Trabalho-Final1-Mariana-Faitanin\\paratecoma-gbif.xlsx")

```

#Análise explioratória
```{r}
para<-read.csv("parat_gbif2.csv", header = T)
